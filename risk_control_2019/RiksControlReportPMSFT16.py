...
class RiksControlReportPMSFT16(Base):

    def __init__(self, env, fund, book):
        self.env = env
        self.team= fund
        self.fund= book
        LogManager('RiksControlReportPMSFT16')
    ...
    def saveSpecificTeamRiskStatus(self, dateStr, specificTeamRiskReportData, teamLimitRawData):
        ...
        for col, limitName in colsDict.items():
            ...
            allRecords += records
        if allRecords:
            self.removeTeamReport(dateStr, 'T16')
...
if __name__ == '__main__':
    env = 'prod'
    riskControlReports = RiksControlReportPMSFT16(env,'PMSF','T16')
    riskControlReports.runWithDateRange('2019-02-19')