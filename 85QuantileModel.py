...
class QuantileModel(Base):
    ...
    def returncalcu(self,result):
        ...
        for i in range(len(result['Returns1'])):
            try:
                returns = -result['Returns1'][i] * result['Pos'][i]+ result['Returns2'][i] * result['Pos'][i]
    ...
    def QStratimplement(self,result, quantiles):
        data = result['spread']
        quantile85 = quantiles[0]
        quantile15 = quantiles[1]
        rolloverdate = result['rolloverTraDate']
        beforeROdate = result['beforeRO']
        weeklylasttradepoint = result['lasttradepoint']
        Buy = [0]
        ...
        for i in range(1, len(data)):
            if (beforeROdate[i] != 0) | (rolloverdate[i] != 0) | (weeklylasttradepoint[i] != 0):
                if flag[-1] == 1:
                    ...
                    Sell.append(0)
                continue
            if data[i - 1] > quantile15 and data[i] < quantile15:
                if flag[-1] != 1:
                    flag.append(1)
                    Buy.append(1)
                    Sell.append(0)
                    ...
                    Buy.append(0)
                    Sell.append(0)
            elif data[i - 1] < quantile85 and data[i] > quantile85:
                if flag[-1] != -1:
                    flag.append(-1)
                    Buy.append(0)
                    Sell.append(1)
    ...
    def QStratoutsample(self,quantiles, df, next1Mpoint, next1Wpoint):
        result = df[(df['DateTime'] >= next1Mpoint) & (df['DateTime'] < next1Wpoint)].copy()
        if len(result) > 0:
            result.loc[:, 'quantile85'] = quantiles[0]
            result.loc[:, 'quantile15'] = quantiles[1]
            result = self.setrollover(result)
            result = self.beforeRO(result)
            result = self.markweeklylasttradepoint(result)
            flag, Buy, Sell = self.QStratimplement(result, quantiles)
            result['Buy'] = Buy
            result['Sell'] = Sell
            result['flag'] = flag
        return result

    def Calquantile(self,next1Wpoint, next1Mpoint, df):
        result = df[(df['DateTime'] >= next1Mpoint) & (df['DateTime'] < next1Wpoint)].copy()
        if len(result)>0:
            res = [np.quantile(result['spread'].values, 0.95), np.quantile(result['spread'].values, 0.05)]
        else:
            res = [0,0]
        return res

    def Stratquantile_engine(self,df):
        startpoint = df['DateTime'].iloc[0]
        endpoint = df['DateTime'].iloc[-1]
        next1Wpoint = startpoint
        resultdf = pd.DataFrame()
        quantile8515 = []
        while (next1Wpoint <= endpoint):
            next1Mpoint = startpoint + dt.timedelta(days=30)
            next1Wpoint = next1Mpoint + dt.timedelta(days=7)
            quantiles = self.Calquantile(next1Wpoint, next1Mpoint, df)
            quantile8515.append(quantiles)
            resultoutsample = self.QStratoutsample(quantiles, df, next1Mpoint, next1Wpoint)
            resultdf = pd.concat([resultdf, resultoutsample], ignore_index=True)
            startpoint = startpoint + dt.timedelta(days=7)
        print(quantile8515)
        return resultdf

    def runQuantileModel(self):
        df1 = pd.read_csv('F:\\maincontract\\J.csv')
        df2 = pd.read_csv('F:\\maincontract\\JM.csv')
        result = self.Kmodelrun(df1, df2)
        result['DateTime'] = pd.to_datetime(result['DateTime'])
        outdata = self.Stratquantile_engine(result)
        result = self.returncalcu(outdata)
        print(result['CumStrategyReturn'].iloc[-1])

if __name__ == '__main__':
    QuantileModel = QuantileModel()
    QuantileModel.runQuantileModel()
